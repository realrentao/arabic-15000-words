...
(function () {
  ...
  var DATA = {};            // gid -> 分册数据
  var AUDIO = "audio/";
  var LS_DONE = "arbook_done", LS_THEME = "arbook_theme", LS_POS = "arbook_pos";
  var LS_SRS = "arbook_srs";
  ...
  function block(title, kind, arr) {
    ...
    arr.forEach(function (it, i) {
      ...
      h += '<div class="row" id="' + id + '"><span class="idx">' + (i + 1) + '</span>'
        ...
        + (it[6] ? '<span class="ipa pron" title="阿语转写">/' + esc(it[6]) + '/</span>' : '')
  ...
  function expand() {
    ...
    if (m === "ar-zh") {
      P.units.forEach(function (u) {
        L.push({ src: AUDIO + u.ae, uid: u.id, lang: "ar" });
        L.push({ src: AUDIO + u.az, uid: u.id, lang: "zh" });
      });
    } else if (m === "all-ar-zh") {
      P.units.forEach(function (u) { L.push({ src: AUDIO + u.ae, uid: u.id, lang: "ar" }); });
  ...
  function init() {
    buildFlat();
    loadLS();
    el("bookTitle").textContent = META.title || "阿拉伯语单词随身背";
  ...
  function renderCard(it, body, foot) {
    body.innerHTML = '<div class="card-face front">'
      ...
      + '<button class="cf-spk spk" data-a="' + AUDIO + it.ae + '" title="阿语发音">🔊 阿语</button>'
  ...
  function exportProgress() {
    ...
    try {
      ...
      a.href = url; a.download = "italian-vocab-progress.json";
